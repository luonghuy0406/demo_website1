...
function ChildPump() {
  ...
  useEffect(() => {
    if (inView && !$("#lb-" + id).hasClass("animate__fadeInLeft")) {
      $("#lb-" + id).addClass("animate__fadeInLeft");
    }
    if (inView && !$("#ct-" + id).hasClass("animate__fadeInRight")) {
      $("#ct-" + id).addClass("animate__fadeInRight");
    }
  }, [inView]);
  return (
...
export default ChildPump;

function SimilarProducts({ ...props }) {
  const classes = useStyles();
  const { t } = useTranslation();
  const { id1: firstId, id2: id } = useParams();
  const { ref, inView, entry } = useInView({
    /* Optional options */
    threshold: 0,
  });
  useEffect(() => {
    if (inView && !$("#lb2-" + id).hasClass("animate__fadeInLeft")) {
      $("#lb2-" + id).addClass("animate__fadeInLeft");
    }
    if (inView && !$("#ct2-" + id).hasClass("animate__fadeInRight")) {
      $("#ct2-" + id).addClass("animate__fadeInRight");
    }
  }, [inView]);
  return (
    <Grid
